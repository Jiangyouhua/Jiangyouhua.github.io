...
window.onload = function () {
    ...
    for (x in elements) {
        elements[x].onsubmit = function (e) {
            ...
            var callback = eval(this.getAttribute("data-back"))
            Web.Post(url, data, callback, "josn")
        }
    }
    ...
    function textarea(element) {
        return [element.name, element.innerHTML]
    }
}
...
var URL = (function () {
    ...
    if (window.location.search != "") {
        obj = Url._searchParse()
    }

    // 解析Url search
    var _searchParse = function () {
...
var Web = {
    ...
    Post: function (url, data, callBack, classify) {
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                    var obj = Web._response(classify, xmlhttp.responseText)
                    callBack(obj)
                    return false
            }
        }
    ...
    Parse: function (selector) {
        ...
        var b = (typeof selector == "string")
        if (!b ){
            // 判断是否文档元素对象
            if (!selector.nodeType || selector.nodeType != 1) {
                return
            }
            if(Web._parseJs(selector)){
                return
            }
        }
    ...
    _parse:function(array){
        ...
        for (var x = 0; x < array.length; x++) {
            ...
            var source = doc.getAttribute("data-source")
            var back = doc.getAttribute("data-back")
            if (!name) {
                Web._parseDom(doc, source)
                continue
            }
            // 解析组件插入
            var config = doc.getAttribute("data-config")
            var func = doc.getAttribute("data-func")
            Web._parsePart(doc, name, source, config, func, back)
        }
    }
    ,
     ...
     * @private
     */
    _parseDom: function (doc, source, back) {
        Web._dataFromParma(source, function (data) {
            ...
            if (doc.tagName == "INPUT" || doc.tagName == "SELECT") {
                doc.setAttribute("value", re)
                return
            }
            if (doc.tagName == "img") {
                doc.setAttribute("src", re)
                return
            }
            doc.innerHTML = re
            Web.Parse(doc)
     ...
     * @private
     */
    _parsePart: function (doc, name, source, config, func, back) {
        var c = Web._dataFromParma(config)
        Web._dataFromParma(source, function (data) {
            // 判断是否有数据处理函数
            var re = !!func ? func(data, doc) : data
            try {
            ...
            // 字符串化JPart
            part.SetIt(doc).SetData(re).SetConfig(c).SetBack(back).Out()
            Web.Parse(doc)
            if (!back) {
                return
            }
    ...
    _dataFromParma: function (param, callback) {
        ...
        if (typeof param != "string") {
            return !callback ? param : callback(param)
        }
...
Part.prototype = {
    ...
    Out: function () {
        ...
        if (!!this._back) {
            try {
                var f = eval(this._back)
                f(this._data, this._it, this._select)
            } catch (e) {
                console.log(e)
            }
        }
